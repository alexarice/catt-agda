...
data _≃p_ : Path S → Path T → Set
data _≃p′_ : Path′ S → Path′ T → Set

data _≃p_ where
  ≃Here : S ≃ S′ → PHere {S = S} ≃p PHere {S = S′}
  ≃Other : P′ ≃p′ Q′ → POther P′ ≃p POther Q′

data _≃p′_ where
  ≃Ext : ∀ {P : Path S} {Q : Path S′} → P ≃p Q → T ≃ T′ → PExt {T = T} P ≃p′ PExt {T = T′} Q
  ≃Next : S ≃ S′ → T ≃ T′ → PNext {S = S} {T = T} ≃p′ PNext {S = S′} {T = T′}
...
≃p-to-same-n (≃Other x) = ≃p′-to-same-n x
...
≃p′-to-same-n (≃Ext p q) = cong₂ (λ a b → a + (2 + b)) (≃-to-same-n q) (≃p-to-same-n p)
≃p′-to-same-n (≃Next p q) = cong₂ (λ a b → a + (2 + b)) (≃-to-same-n q) (≃-to-same-n p)
≃p′-to-same-n (≃Shift p q) = cong₂ (λ a b → a + (2 + b)) (≃p′-to-same-n q) (≃-to-same-n p)
...
path-to-term-≃ (≃Other x) = path′-to-term-≃ x
...
path′-to-term-≃ (≃Ext p q) = sub-action-≃-tm (susp-tm-≃ (path-to-term-≃ p)) (connect-susp-inc-left-≃ (≃p-to-same-n p) (≃-to-same-n q))
path′-to-term-≃ (≃Next p q) = sub-action-≃-tm (Var≃ (cong suc (≃-to-same-n q)) (cong (toℕ ∘ fromℕ) (≃-to-same-n q))) (connect-susp-inc-right-≃ (≃-to-same-n p) (≃-to-same-n q) )
path′-to-term-≃ (≃Shift p q) = sub-action-≃-tm (path′-to-term-≃ q) (connect-susp-inc-right-≃ (≃-to-same-n p) (≃p′-to-same-n q))

refl≃p : P ≃p P
refl≃p′ : P′ ≃p′ P′

refl≃p {P = PHere} = ≃Here refl≃
refl≃p {P = POther x} = ≃Other refl≃p′

refl≃p′ {P′ = PExt x} = ≃Ext refl≃p refl≃
refl≃p′ {P′ = PNext} = ≃Next refl≃ refl≃
...
sym≃p (≃Other x) = ≃Other (sym≃p′ x)

sym≃p′ (≃Ext p q) = ≃Ext (sym≃p p) (sym≃ q)
sym≃p′ (≃Next p q) = ≃Next (sym≃ p) (sym≃ q)
sym≃p′ (≃Shift p q) = ≃Shift (sym≃ p) (sym≃p′ q)

trans≃p : P ≃p Q → Q ≃p R → P ≃p R
trans≃p′ : P′ ≃p′ Q′ → Q′ ≃p′ R′ → P′ ≃p′ R′

trans≃p (≃Here x) (≃Here y) = ≃Here (trans≃ x y)
trans≃p (≃Other x) (≃Other y) = ≃Other (trans≃p′ x y)

trans≃p′ (≃Ext p x) (≃Ext q y) = ≃Ext (trans≃p p q) (trans≃ x y)
trans≃p′ (≃Next w x) (≃Next y z) = ≃Next (trans≃ w y) (trans≃ x z)
...
ppath-≃-≃p (Join≃′ p q) (POther x) = ≃Other (ppath′-≃-≃p′ (Join≃′ p q) x)

ppath′-≃-≃p′ Refl≃′ P′ = refl≃p′
ppath′-≃-≃p′ (Join≃′ p q) (PExt x) = ≃Ext {!ppath′-≃-≃p′!} {!!}
ppath′-≃-≃p′ (Join≃′ p q) PNext = {!!}
ppath′-≃-≃p′ (Join≃′ p q) (PShift P′) = {!!}
