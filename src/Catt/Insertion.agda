...
interior-sub : (S : Tree n) → (P : Path S) → (T : Tree m) → .(lh : has-linear-height (path-length P) T) → Sub (suc m) (suc (insertion-tree-size S P T lh))
interior-sub S (PHere .S) T lh = idSub (suc _)
interior-sub (Join S₁ S₂) (PExt P) (Join T Sing) lh = connect-inc-left (suspCtx (tree-to-ctx (insertion-tree S₁ P T lh))) (getSnd) (tree-to-ctx S₂) ∘ suspSub (interior-sub S₁ P T lh)
interior-sub (Join S₁ S₂) (PShift P) T lh = connect-inc-right (suspCtx (tree-to-ctx S₁)) (getSnd) (tree-to-ctx (insertion-tree S₂ P T lh)) ∘ interior-sub S₂ P T lh
