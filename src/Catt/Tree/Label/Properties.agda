...
_≃l_ : (L : Label X S A) → (M : Label Y S B) → Set
_≃l_ {S = S} L M = (Q : PPath S) → apt L Q ≃tm apt M Q

label-to-sub-≃ : (L : Label X S A) → (M : Label Y S B) → (L ≃l M) → A ≃ty B → label-to-sub L ≃s label-to-sub M
label-to-sub-≃ {S = Sing} L M p q = Ext≃ (Null≃ q) (p PPHere)
label-to-sub-≃ {S = Join S T} L M p q
  = sub-from-connect-≃ (unrestrict-≃ (label-to-sub-≃ (label₁ L) (label₁ M) (λ P → p (PPExt P)) (Arr≃ (p PPHere) q (p (PPShift PPHere)))))
...
susp-label-to-sub {S = Join S T} {A = A} L = begin
  < sub-from-connect (unrestrict (label-to-sub (label₁ (susp-label L))))
    ...
    ≈⟨ sub-from-connect-≃ (unrestrict-≃ (label-to-sub-≃ (label₁ (susp-label L)) (susp-label (label₁ L)) (λ P → refl≃tm) (Arr≃ (susp-path-to-term (ap L PPHere))
                                                                                                                              refl≃ty
...
id-label-to-sub (Join S T) = begin
  ...
  where
    ...
    l1 = begin
      < unrestrict (label-to-sub (label₁ (id-label (Join S T)))) >s
        ≈˘⟨ unrestrict-≃ (label-to-sub-≃ (label-sub (susp-label (id-label S))
                                         ...
                                         (λ P → sub-action-≃-tm (id-on-tm (suspTm (path-to-term (carrier P)))) refl≃s)
    ...
    l2 = begin
      < label-to-sub (label₂ (id-label (Join S T))) >s
        ≈⟨ label-to-sub-≃ (label₂ (id-label (Join S T))) (label-sub (id-label T) (someTree (Join S T)) (connect-susp-inc-right (tree-size S) (tree-size T))) (λ P → refl≃tm) refl≃ty ⟩
...
connect-label-inc-left {S = Join S₁ S₂} L M ⟦ PExt Q ⟧ = path-to-term-≃ (extend-ppath ⟦ PExt Q ⟧ L)
...
-- connect-label-prop : (S : Tree n) → (T : Tree m) → (Q : PPath (connect-tree S T)) → connect-label (connect-tree-inc-left S T) (connect-tree-inc-right S T) .ap Q ≃p carrier Q
-- connect-label-prop Sing T ⟦ PHere ⟧ = refl≃p
-- connect-label-prop Sing .(Join _ _) ⟦ PExt Q ⟧ = refl≃p
-- connect-label-prop Sing .(Join _ _) ⟦ PShift Q ⟧ = refl≃p
-- connect-label-prop (Join S₁ S₂) T ⟦ PHere ⟧ = refl≃p
-- connect-label-prop (Join S₁ S₂) T ⟦ PExt Q ⟧ = refl≃p
-- connect-label-prop (Join S₁ S₂) T ⟦ PShift Q ⟧ = {!!}
