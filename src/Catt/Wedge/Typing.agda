...
sub-from-wedge-inc-right-≈ : (σ : Sub (suc n) l A) → (t : Tm (suc n)) → (τ : Sub (suc m) l A) → {Γ : Ctx l} → (t [ σ ]tm ≈[ Γ ]tm Var (fromℕ _) [ τ ]tm) → sub-from-wedge σ τ ● wedge-inc-right t m ≈[ Γ ]s τ
...
sub-from-wedge-inc-right-≈ σ t ⟨ ⟨ τ , s ⟩ , u ⟩ p = Ext≈ lem refl≈tm
  where
    ...
    lem : sub-from-wedge σ ⟨ ⟨ τ , s ⟩ , u ⟩ ● lift-sub (wedge-inc-right t _) ≈[ _ ]s ⟨ τ , s ⟩
    lem = begin
...
sub-from-wedge-Ty {Υ = Υ} {σ = σ} {t = t} σty tty (TyExt {A = A} (TyExt {σ = τ} {t = s} τty y) x) p = TyExt (sub-from-wedge-Ty σty tty (TyExt τty y) p) (TyConv x lem)
  where
    ...
    lem = begin
      ...
      A [ sub-from-wedge σ ⟨ τ , s ⟩ ● wedge-inc-right t _ ]ty
...
sub-between-wedges-Ty {Δ = Δ} {σ = σ} {t = t} {Θ = Θ} {τ = τ} {s = s} σty tty τty sty p q
  ...
  t [ wedge-inc-left s _ ● σ ]tm
    ≈⟨ reflexive≈tm (assoc-tm (wedge-inc-left s _) σ t) ⟩
  ...
  Var (fromℕ _) [ τ ]tm [ wedge-inc-right s _ ]tm
    ≈˘⟨ reflexive≈tm (assoc-tm (wedge-inc-right s _) τ (Var (fromℕ _))) ⟩
  Var (fromℕ _) [ wedge-inc-right s _ ● τ ]tm ∎)
  where
...
between-wedge-from-wedge-≈ : (σ : Sub (suc n) (suc l) ⋆)
                               ...
                               → sub-from-wedge σ′ τ′ ● sub-between-wedges σ τ s ≈[ Γ ]s sub-from-wedge (σ′ ● σ) (τ′ ● τ)
  ...
  where
    ...
    tm-lem = begin
      t [ wedge-inc-right s _ ]tm [ sub-from-wedge σ′ τ′ ]tm
        ≈˘⟨ reflexive≈tm (assoc-tm (sub-from-wedge σ′ τ′) (wedge-inc-right s _) t) ⟩
      t [ sub-from-wedge σ′ τ′ ● wedge-inc-right s _ ]tm
