...
sub-from-disc-to-term-Ty {A = A} d p (TyExt _ _ tty) = term-conversion tty (reflexive≈ty (sub-from-sphere-prop d A p))
...
sub-from-sphere-to-ty-Ty {A = s ─⟨ A ⟩⟶ t} (suc d) p (TyExt (TyExt σty _ sty) _ tty)
  = TyArr (term-conversion sty (reflexive≈ty (sub-from-sphere-prop d A (cong pred p))))
          (sub-from-sphere-to-ty-Ty d (cong pred p) σty)
          (term-conversion tty (reflexive≈ty (trans≃ty (lift-sub-comp-lem-ty (sub-from-sphere d A _) (sphere-type d)) (sub-from-sphere-prop d A (cong pred p)))))

identity-to-term-Ty : Typing-Tm Γ (identity t A) B → Typing-Tm Γ t A
identity-to-term-Ty (TyCoh Aty σty _ _ _) = sub-from-disc-to-term-Ty (ty-dim _) refl σty

identity-to-type-Ty : Typing-Tm Γ (identity t A) B → Typing-Ty Γ A
identity-to-type-Ty (TyCoh Aty (TyExt σty _ _) _ _ _) = sub-from-sphere-to-ty-Ty (ty-dim _) refl σty
