...
sub-from-linear-tree-≃ (Sing≃ _) b (Star≃ x) p q = Ext≃ (Null≃ (Star≃ x)) b
sub-from-linear-tree-≃ (Join≃ _ a (Sing≃ _)) b (Arr≃ c d e) p q = Ext≃ (Ext≃ (sub-from-linear-tree-≃ a c d (cong pred p) (cong pred q)) e) b
...
sub-from-linear-tree-sub (Sing≃ _) t A p σ = refl≃s
sub-from-linear-tree-sub (Join≃ _ a (Sing≃ _)) t (s ─⟨ A ⟩⟶ s′) p σ = Ext≃ (Ext≃ (sub-from-linear-tree-sub a s A (cong pred p) σ) refl≃tm) refl≃tm
