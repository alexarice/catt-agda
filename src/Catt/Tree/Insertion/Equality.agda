...
exterior-unbiased-comp : (S : Tree n)
                  ...
                  → .⦃ _ : has-linear-height l T ⦄
                  → (tree-dim T ≤ height-of-branching P)
                  → (unbiased-comp (tree-dim S) S >>= exterior-sub-label S P T ,, S⋆)
                  ≈[ incTree (insertion-tree S P T) ]stm unbiased-comp (tree-dim S) (insertion-tree S P T)
exterior-unbiased-comp S P T p = begin
  SCoh S (unbiased-type (tree-dim S) S) (exterior-sub-label S P T ,, S⋆)
    ≈⟨ {!!} ⟩
  SCoh (insertion-tree S P T) (label-on-sty (unbiased-type (tree-dim S) S) (exterior-sub-label S P T ,, S⋆)) (sub-from-insertion-label S P T (exterior-sub-label S P T) (interior-sub-label S P T) ,, S⋆)
    ≈⟨ ≈SCoh (insertion-tree S P T) (exterior-unbiased-type S P T (tree-dim S) (n≤1+n (tree-dim S)) p) (reflexive≈l (exterior-interior-prop S P T)) refl≈sty ⟩
  SCoh (insertion-tree S P T) (unbiased-type (tree-dim S) (insertion-tree S P T)) (id-label-wt _) ∎
  where
    open Reasoning (stm-setoid-≈ (incTree (insertion-tree S P T)))
