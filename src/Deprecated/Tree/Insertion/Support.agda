module Deprecated.Tree.Insertion.Support where
...
open import Deprecated.Typing.Structured.Support All Weak-Rules (weak-tame all-tame) weak-supp
...
κ-boundary-vs : (S : Tree n)
              ...
              → (d : ℕ)
              → (b : Bool)
              → tree-bd-vs d S b [ κ S p T ]vl ≡ toVarSet (tree-bd-vs d (S >>[ p ] T) b)
