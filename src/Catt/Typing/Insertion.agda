...
HasInsertion = ∀ {m n l n′}
             ...
             → (M : Label X T)
             → L ⌊ P ⌋p ≃stm (standard-coh′ (ih P) T >>= (M ,, S⋆))
             → {Cs : STy X}
...
ins-from-rule p {S = S} As L P {T = T} M pf [ tty ] .get = begin
  ...
  where
    lem : label-to-other L ⌊ P ⌋p
          ≃stm
          standard-coh′ (ih P) T >>= (label-to-other M ,, S⋆)
    lem = begin
      < label-to-other L ⌊ P ⌋p >stm
        ≈⟨ ap-≃ (label-to-other-prop L) refl≃p ⟩
      < L ⌊ P ⌋p >stm
        ≈⟨ pf ⟩
      < standard-coh′ (ih P) T >>= (M ,, S⋆) >stm
        ≈˘⟨ >>=-≃ (refl≃stm {a = standard-coh′ (ih P) T}) (label-to-other-prop M) (sty-to-other-prop S⋆) ⟩
      < standard-coh′ (ih P) T >>= (label-to-other M ,, S⋆) >stm ∎
      where
