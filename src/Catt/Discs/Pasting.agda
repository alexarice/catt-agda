...
disc-pdb-focus-ty : (d : ℕ) → focus-ty (disc-pdb d) ≃ty wk-ty (sphere-type d)
disc-pdb-focus-tm : (d : ℕ) → focus-tm (disc-pdb d) ≃tm 0V {n = disc-size d}

disc-pdb zero = Base
disc-pdb (suc d) = Extend (disc-pdb d) (sym≃ty (disc-pdb-focus-ty d)) (sym≃ty (Arr≃ (wk-tm-≃ (disc-pdb-focus-tm d)) (wk-ty-≃ (disc-pdb-focus-ty d)) refl≃tm))
