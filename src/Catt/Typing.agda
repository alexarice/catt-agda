...
data _≈tm_ where
  Var≈ : Γ ≈c Δ → {i : Fin (ctxLength Γ)} → {j : Fin (ctxLength Δ)} → (toℕ i ≡ toℕ j) → (Var {Γ = Γ} i) ≈tm (Var {Γ = Δ} j)
  Sym≈ : s ≈tm t → t ≈tm s
...
data _≈ty_ where
  Star≈ : Γ ≈c Δ → _≈ty_ {Γ = Γ} {Δ = Δ} ⋆ ⋆
  Arr≈ : s ≈tm s′ → A ≈ty A′ → t ≈tm t′ → (s ─⟨ A ⟩⟶ t) ≈ty (s′ ─⟨ A′ ⟩⟶ t′)

data _≈s_ where
  Null≈ : Δ ≈c Δ′ → _≈s_ {Δ = Δ} {Δ′ = Δ′} ⟨⟩ ⟨⟩
...
data Typing-Tm where
  TyVar : (i : Fin (ctxLength Γ)) → {B : Ty Γ d} → (Γ ‼ i) ≈ty B → Typing-Tm Γ (Var i) B
