...
connect-inc-right : (Γ : Ctx (suc n)) → (x : Tm (suc n)) → (Δ : Ctx (suc m)) → Sub (suc m) (suc (m + n))
...
connect-inc-left : (Γ : Ctx (suc n)) → (x : Tm (suc n)) → (Δ : Ctx (suc m)) → Sub (suc n) (suc (m + n))
connect-inc-left Γ x (∅ , A) = idSub (suc _)
