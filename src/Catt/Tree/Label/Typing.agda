...
module _ (r : SRule) where
  ...
  srule-lift f tty = begin
    liftTerm (stm-to-term lhs)
      ≈˘⟨ reflexive≈tm (lift-stm-to-term lhs) ⟩
    stm-to-term (lift-stm lhs)
      ≈⟨ f [ transport-typing-full tty (sym≃tm (lift-stm-to-term lhs)) (sym≃ty (trans≃ty (lift-sty-to-type (to-sty _)) (lift-ty-≃ (to-sty-to-type _)))) ] .get ⟩
    stm-to-term (lift-stm rhs)
      ≈⟨ reflexive≈tm (lift-stm-to-term rhs) ⟩
    liftTerm (stm-to-term rhs) ∎
    where
      open Reasoning (tm-setoid-≈ _)

  srule-susp : (∀ {C : STy (Other len)}
               → Typing-STm (suspCtx tgtCtx) (susp-stm lhs) (susp-sty C)
               → susp-stm lhs ≈[ suspCtx tgtCtx ]stm susp-stm rhs)
               → SuspRule (srule-to-rule r)
  srule-susp f tty = begin
    suspTm (stm-to-term lhs)
      ≈˘⟨ reflexive≈tm (susp-stm-to-term lhs) ⟩
    stm-to-term (susp-stm lhs)
      ≈⟨ f [ transport-typing-full tty (sym≃tm (susp-stm-to-term lhs)) (sym≃ty (trans≃ty (susp-sty-to-type (to-sty _)) (susp-ty-≃ (to-sty-to-type _)))) ] .get ⟩
    stm-to-term (susp-stm rhs)
      ≈⟨ reflexive≈tm (susp-stm-to-term rhs) ⟩
    suspTm (stm-to-term rhs) ∎
    where
      open Reasoning (tm-setoid-≈ _)
  ...
  srule-sub : (∀ {n}
              ...
              → Typing-STm Δ (stm-sub lhs σ) (sty-sub C σ)
              → stm-sub lhs σ ≈[ Δ ]stm stm-sub rhs σ)
              → SubRule (srule-to-rule r)
  srule-sub f {σ = σ} {C = C} σty tty = f {C = to-sty C} σty [ TyConv tty (reflexive≈ty (sym≃ty (trans≃ty (sty-sub-prop (to-sty C) σ) (sub-action-≃-ty (to-sty-to-type C) (refl≃s {σ = σ}))))) ] .get
