...
linear-tree-unbiased-lem (suc d) (Join S Sing) p = begin
  ...
  < suspTy (tree-to-ctx S ‼ zero) >ty
    ≈⟨ susp-ty-≃ (linear-tree-unbiased-lem d S (sym (max-pred-0 (sym p)))) ⟩
...
sub-from-linear-tree-unbiased-0V (Join S Sing) T d = trans≃tm (sym≃tm (unrestrict-comp-tm 0V (sub-from-linear-tree-unbiased S T (suc d)))) (trans≃tm (sub-from-linear-tree-unbiased-0V S T (suc d)) (unbiased-comp-≃ (trans (sym (+-suc d (tree-dim S))) (cong (λ - → d + suc -) (sym (max-lem (tree-dim S))))) refl≃))
...
sub-from-linear-tree-‼-0 (Join S Sing) t (s ─⟨ A ⟩⟶ s′) p = begin
  < suspCtx (tree-to-ctx S) ‼ zero [ ⟨ ⟨ sub-from-linear-tree S s A (max-pred-0 p) , s′ ⟩ , t ⟩ ]ty >ty
    ≈⟨ sub-action-≃-ty (‼-≃ zero zero refl (susp-lin-tree S)) refl≃s ⟩
  < ((0V [ sub-from-linear-tree S s A (max-pred-0 p) ]tm)
    ─⟨ liftType (liftType (tree-to-ctx S ‼ zero))
       [ ⟨ ⟨ sub-from-linear-tree S s A (max-pred-0 p) , s′ ⟩ , t ⟩ ]ty
       ⟩⟶ s′) >ty
    ≈⟨ Arr≃ (sub-from-linear-tree-0V S s A (max-pred-0 p)) lem refl≃tm ⟩
  < s ─⟨ A ⟩⟶ s′ >ty ∎
  where
    open Reasoning ty-setoid
    ...
    lem = begin
      ...
      < tree-to-ctx S ‼ zero [ sub-from-linear-tree S s A _ ]ty >ty
        ≈⟨ sub-from-linear-tree-‼-0 S s A (max-pred-0 p) ⟩
      < A >ty ∎
...
sub-from-linear-tree-≃ (Join≃ a Sing≃) b (Arr≃ c d e) p q = Ext≃ (Ext≃ (sub-from-linear-tree-≃ a c d (max-pred-0 p) (max-pred-0 q)) e) b
...
sub-from-linear-tree-sub (Join≃ a Sing≃) t (s ─⟨ A ⟩⟶ s′) p σ = Ext≃ (Ext≃ (sub-from-linear-tree-sub a s A (max-pred-0 p) σ) refl≃tm) refl≃tm

identity-sub : (t : Tm n) → (A : Ty n) → (σ : Sub n m ⋆) → identity t A [ σ ]tm ≃tm identity (t [ σ ]tm) (A [ σ ]ty)
identity-sub t A σ = Coh≃ (tree-to-ctx-≃ (n-disk-≃ (sub-dim σ A))) (unbiased-type-≃ (cong suc (sub-dim σ A)) (n-disk-≃ (sub-dim σ A))) (sub-from-linear-tree-sub (n-disk-≃ (sub-dim σ A)) ⦃ n-disk-is-linear (ty-dim A) ⦄ ⦃ n-disk-is-linear (ty-dim (A [ σ ]ty)) ⦄ t A (sym (tree-dim-n-disk (ty-dim A))) σ)

identity-≃ : s ≃tm t → A ≃ty B → identity s A ≃tm identity t B
identity-≃ {A = A} {B = B} p q = Coh≃ (tree-to-ctx-≃ (n-disk-≃ (ty-dim-≃ q))) (unbiased-type-≃ (cong suc (ty-dim-≃ q)) (n-disk-≃ (ty-dim-≃ q))) (sub-from-linear-tree-≃ (n-disk-≃ (ty-dim-≃ q)) ⦃ n-disk-is-linear (ty-dim A) ⦄ ⦃ n-disk-is-linear (ty-dim B) ⦄ p q (sym (tree-dim-n-disk (ty-dim A))) (sym (tree-dim-n-disk (ty-dim B))))
